...
const slides: SlideData[] = [
  {
    ...
    icon: "dumbbell",
    description:
      "Not someday. Today. The pyramid builds you up one rep at a time.",
  ...
  },
  {
    ...
    icon: "chart-up",
    description:
      "Short breaks when it's easy. Longer when it's hard. You're always ready for the next set.",
  ...
  },
  {
    ...
    icon: "waveform",
    description:
      "Audio cues for every rep. Get in position, close your eyes, and go.",
  ...
  return (
    <View className="flex-1 items-center justify-center px-8" style={{ width }}>
      <View className="items-center">
        <View className="mb-6">
          <Icon color="#a3e635" name={data.icon} size={72} />
        </View>
        ...
        {data.highlight && (
          <View className="mt-4 rounded-full bg-zinc-800 px-4 py-2">
            <Text className="font-mono text-lime-400 text-sm">
              {data.highlight}
            </Text>
          </View>
        )}
        {data.quote && (
          <View className="mt-6 max-w-xs">
            <Text className="text-center text-base text-zinc-500 italic">
              "{data.quote.text}"
            </Text>
            <Text className="mt-2 text-center font-semibold text-sm text-zinc-600">
              — {data.quote.author}
            </Text>
          </View>
        )}
  ...
  return (
    <View className="flex-1 bg-zinc-950">
      ...
      <View className="absolute right-4 z-10" style={{ top: insets.top + 16 }}>
        <Pressable className="rounded-full px-4 py-2" onPress={handleSkip}>
      ...
      >
        {slides.map((slide, index) => (
          <Slide data={slide} key={index} width={width} />
        ))}
      </ScrollView>
      ...
      >
        ...
        <View className="flex-row gap-2">
          {slides.map((_, idx) => (
            <Dot active={idx === activeIndex} key={idx} />
          ))}
        </View>
